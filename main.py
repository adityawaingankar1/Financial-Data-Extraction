...
# NEW: local degraded fallback (spaCy + regex)
def _fallback_simple_entities(text: str) -> Dict[str, Any]:
    """
    Local degraded mode: combine spaCy NER (if available) with finance regex rules.
    Returns a dict with entities + optional spacy visualization HTML.
    """
    ...
    if SPACY_AVAILABLE and SPACY_NLP is not None:
        try:
            ...
            logger.warning("spaCy degraded extraction failed: %s", e)

    # Regex entities
    for m in run_regex_on_text(text or ""):
        _add_non_overlapping_span(manual_ents, m["start"], m["end"], m["label"].upper())

    manual_ents.sort(key=lambda x: (x["start"], x["end"]))
    ents_for_table = []
    labels_used = set()
    for e in manual_ents:
        span_text = (text or "")[e["start"]: e["end"]]
        ents_for_table.append({
            "text": span_text,
            "label": e["label"],
            "start": e["start"],
            "end": e["end"],
            "page": "-",
            "attributes": {},  # keep shape consistent with LangExtract
        })
    ...
    try:
        if displacy:
            displacy_input = [{"text": text or "", "ents": manual_ents, "title": None}]
            spacy_html = displacy.render(displacy_input, style="ent", manual=True, page=True, options={"colors": SPACY_COLORS})
    except Exception as e:
        logger.warning("displaCy render failed in degraded mode: %s", e)
...
# NEW: resilient LangExtract call with fast fallback, time budget, provider-aware API keys
def _lx_extract_with_retry_and_fallback(
    text: str,
    ...
    """
    Provider-aware fallbacks with fast handling of quota (429) and a time budget.
    429 with quota/resource_exhausted => non-retryable; immediately try next provider.
    Long Retry-After values are clamped or skipped based on budget.
    """
    if not LANGEXTRACT_AVAILABLE:
        raise RuntimeError("LangExtract not installed.")

    def _provider_key_for_model(m: str) -> Optional[str]:
        ml = (m or "").lower()
        if ml.startswith("gemini"):
            return os.getenv("GOOGLE_API_KEY") or os.getenv("LANGEXTRACT_API_KEY") or api_key
        if ml.startswith("gpt") or "openai" in ml:
            return os.getenv("OPENAI_API_KEY")
        # Router does not auto-handle 'ollama/...'; disabled below by default
        return api_key

    primary = (model_id or os.getenv("LANGEXTRACT_DEFAULT_MODEL", "gemini-2.5-pro")).strip()
    chain: List[str] = [primary]

    # Optional env fallbacks
    extra_fallbacks = os.getenv("LANGEXTRACT_FALLBACK_MODELS", "")
    if extra_fallbacks:
        for m in [x.strip() for x in extra_fallbacks.split(",") if x.strip()]:
            if m not in chain:
                chain.append(m)

    # Gemini alternates (SDK v1beta/v1 compatibility varies by project)
    if primary.lower().startswith("gemini"):
        for alt in ["gemini-1.5-pro", "gemini-1.5-flash-001", "gemini-1.5-flash-latest"]:
            if alt not in chain:
                chain.append(alt)

    # OpenAI fallback if key is present
    if os.getenv("OPENAI_API_KEY"):
        for alt in ["gpt-4o-mini", "gpt-4o-mini-2024-07-18"]:
            if alt not in chain:
                chain.append(alt)

    # Ollama disabled by default (router won’t resolve 'ollama/...').
    # Enable only if you wire a provider config in LangExtract.
    if os.getenv("LANGEXTRACT_ALLOW_OLLAMA", "0") == "1":
        for alt in ["ollama/llama3.1:8b", "ollama/llama3.2:3b"]:
            if alt not in chain:
                chain.append(alt)

    # Concurrency
    max_workers = int(os.getenv("LANGEXTRACT_MAX_WORKERS", "1"))
    batch_length = int(os.getenv("LANGEXTRACT_BATCH_LENGTH", "1"))
    # Attempts per model (keep low to avoid UI hangs)
    per_model_retries = int(os.getenv("LANGEXTRACT_PER_MODEL_RETRIES", "1"))

    # Fast-fallback tuning
    max_retry_after_s = float(os.getenv("LANGEXTRACT_MAX_RETRY_AFTER_S", "3"))   # clamp long Retry-After
    total_budget_s = float(os.getenv("LANGEXTRACT_TOTAL_BUDGET_S", "25"))        # overall time budget
    start_ts = time.time()

    last_err: Optional[Exception] = None

    def _parse_retry_delay_seconds(msg: str) -> Optional[float]:
        m = re.search(r"retry in (\d+(?:\.\d+)?)s", msg.lower())
        if m:
    ...
    def _is_quota_exhausted(msg: str) -> bool:
        ml = msg.lower()
        return ("429" in ml) and any(tok in ml for tok in ["quota", "resource_exhausted", "free tier", "free_tier", "exceeded your current quota"])

    for m in chain:
        for attempt in range(per_model_retries):
            # Budget check
            if (time.time() - start_ts) > total_budget_s:
                logger.warning("LangExtract: total time budget exceeded; stopping retries.")
                raise last_err or RuntimeError("Time budget exceeded")

            try:
                logger.info("LangExtract: using model '%s' (attempt %d/%d)", m, attempt + 1, per_model_retries)
                provider_key = _provider_key_for_model(m)
                res = lx.extract(
                ...
                if m != primary:
                    logger.info("LangExtract: succeeded with fallback model '%s'", m)
                return res, m
            except Exception as e:
                last_err = e
                msg = str(e)

                # Quota (daily/plan) => non-retryable; jump to next provider immediately
                if _is_quota_exhausted(msg):
                    logger.warning("LangExtract: quota exhausted for '%s'. Skipping further retries for this provider.", m)
                    break  # go to next provider

                # Retry-eligible transient
                transient_tokens = ["503", "unavailable", "overloaded", "temporarily", "rate limit", "rate-limit"]
                if any(tok in msg.lower() for tok in transient_tokens) or "429" in msg:
                    retry_after = _parse_retry_delay_seconds(msg)
                    # If provider asks for a long wait, prefer fast fallback
                    if retry_after and retry_after > max_retry_after_s:
                        logger.warning("LangExtract: Retry-After=%.2fs too long. Fast-fallback to next provider.", retry_after)
                        break
                    sleep_s = min(max_retry_after_s, (retry_after or 0) or min(2 ** attempt, max_retry_after_s))
                    # Check time budget before sleeping
                    if (time.time() - start_ts + sleep_s) > total_budget_s:
                        logger.warning("LangExtract: sleep would exceed time budget. Moving to next provider.")
                        break
                    logger.warning("LangExtract: transient error (%s). Retrying in %ss...", msg, round(sleep_s, 2))
                    time.sleep(sleep_s)
                    continue

                logger.warning("LangExtract: non-retryable error with model '%s': %s", m, msg)
                break  # move to next provider

        logger.info("LangExtract: moving to next fallback after failures with '%s'", m)

    raise last_err or RuntimeError("LangExtract failed after retries and fallbacks.")
...
async def process_file(
    ...
    try:
        ...
        if model_choice.lower() == "langextract":
            ...
            if not LANGEXTRACT_AVAILABLE:
                degraded = _fallback_simple_entities(doc_text or "")
                return JSONResponse(
                    {
                        "status": "success",
                        "model": "langextract",
                        "degraded": True,
                        "reason": "LangExtract not installed. Returned degraded local extraction.",
                        "text": doc_text,
                        "entities": degraded["entities"],
                        "file_type": (suffix.lower().lstrip(".") if suffix else "pdf"),
                        "pdf_highlights": [],
                        "pdf_url": f"/download/{tmp_path.name}" if tmp_path else None,
                        "visualization_html": degraded.get("spacy_visualization_html", ""),
                        "spacy_visualization_html": degraded.get("spacy_visualization_html", ""),
                        "spacy_labels_used": degraded.get("labels_used", []),
                    },
                    status_code=200,
                )

            try:
                lx_result, _used_model = _lx_extract_with_retry_and_fallback(
                    text=doc_text or "",
                    prompt_description=prompt or default_prompt,
                    examples=example_objs if example_objs else None,
                    model_id=(model_id or os.getenv("LANGEXTRACT_DEFAULT_MODEL", "gemini-2.5-pro")),
                    api_key=None,
                )
                ...
                logger.exception("LangExtract failed, returning degraded local extraction.")
                degraded = _fallback_simple_entities(doc_text or "")
                return JSONResponse(
                    {
                        "status": "success",
                        "model": "langextract",
                        "degraded": True,
                        "reason": f"LangExtract failed: {str(e)}",
                        "text": doc_text,
                        "entities": degraded["entities"],
                        "file_type": (suffix.lower().lstrip(".") if suffix else "pdf"),
                        "pdf_highlights": [],
        ...
        if model_choice.lower() in ["bert", "finbert", "bert/finbert"]:
            if not FINBERT_READY:
                ...
                return JSONResponse({"status": "error", "message": msg}, status_code=200)
            ...
            active_model = FINBERT_MODEL_NAME or "ProsusAI/finbert"
            model_lower = active_model.lower()
            is_tone = ("yiyanghkust" in model_lower) or ("finbert-tone" in model_lower)

            if is_tone:
                bull_pos_threshold = float(os.getenv("FINBERT_TONE_POS_RATIO", "0.55"))
                bull_neg_cap = float(os.getenv("FINBERT_TONE_NEG_CAP", "0.30"))
                bear_neg_threshold = float(os.getenv("FINBERT_TONE_BEAR_NEG_RATIO", "0.45"))
                rec_bull = "Tone indicates constructive, opportunity-focused language. Advance, but capture rationale and stakeholder tone for comms."
                rec_bear = "Tone suggests caution (hedging/uncertainty). Address tone drivers and confirm mitigating details."
                rec_mix = "Tone is mixed. Clarify hedging language and align on risk wording; prioritize sections with stronger negative tone."
            else:
                bull_pos_threshold = float(os.getenv("FINBERT_PROSUS_POS_RATIO", "0.60"))
                bull_neg_cap = float(os.getenv("FINBERT_PROSUS_NEG_CAP", "0.25"))
                bear_neg_threshold = float(os.getenv("FINBERT_PROSUS_BEAR_NEG_RATIO", "0.50"))
                rec_bull = "Bullish tone with consistently positive language across sentences. Proceed to next milestone; validate commercial terms before sign-off."
                rec_bear = "Bearish tone with prevalent negative language indicating higher risk. Escalate for deeper review and negotiate the most negative sections."
                rec_mix = "Mixed/neutral tone. Hold targeted review on confident negatives; confirm obligations, timelines, and contingencies."

            if pos_ratio >= bull_pos_threshold and neg_ratio <= bull_neg_cap:
                outlook = "Bullish tone"
                recommendation = rec_bull
            elif neg_ratio >= bear_neg_threshold:
                outlook = "Bearish tone"
                recommendation = rec_bear
            else:
                outlook = "Mixed/neutral tone"
                recommendation = rec_mix
            ...
            predictive_analysis = (
                ...
                f"{outlook} (calibrated thresholds: bullish≥{bull_pos_threshold:.2f}, bearish≥{bear_neg_threshold:.2f}).\n"
            )
            ...
            return JSONResponse(
                {
                    "status": "success",
                    "model": "bert",
                    "bert_model_used": active_model,
                    "text": doc_text,
                    "entities": sentiment_rows,
                    "file_type": (suffix.lower().lstrip(".") if suffix else "pdf"),
                    "pdf_highlights": dedup_highlights,
                    "sentiment_highlights": dedup_highlights,
                    "pdf_url": f"/download/{tmp_path.name}" if tmp_path else None,
                    ...
                    "sentiment_summary": {
                        ...
                        "thresholds_used": {
                            "bullish_pos_ratio": bull_pos_threshold,
                            "bullish_neg_cap": bull_neg_cap,
                            "bearish_neg_ratio": bear_neg_threshold,
                        },
